...
static int
wpa_driver_nl80211_finish_drv_init(struct i802_bss *bss, const u8 *set_addr/*接口要被设置的mac地址*/,
				   const char *driver_params,
				   enum wpa_p2p_mode p2p_mode)
{
	struct wpa_driver_nl80211_data *drv = bss->drv;
...
static int nl80211_setup_ap(struct i802_bss *bss)
{
	...
	if (!drv->device_ap_sme/*如果创建monitor失败，则此值为1*/ && nl80211_mgmt_subscribe_ap(bss))
		return -1;
